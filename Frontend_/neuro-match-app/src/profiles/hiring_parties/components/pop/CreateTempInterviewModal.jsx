...
import { createTempInterview } from "../../../../api/Interview";

const CreateTempInterviewModal = ({ onClose, candidateData, jobPostId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await createTempInterview(
        email, 
      ...
      alert(res.message || `Temporary interview created for ${candidateData.candidate_name}`);
      onClose();
    } catch (err) {
      alert("Failed to create temp interview. Check console.");
      console.error(err);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <section>
           ...
           <input
             ...
             className="w-full px-4 py-2 border-b-2 border-gray-100 focus:border-indigo-500 focus:outline-none transition-colors"
...
export default CreateTempInterviewModal;